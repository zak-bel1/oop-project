...
# Palette de couleurs pour les modulecs (Format: "Couleur Normale", "Couleur Survol")
MODULE_PALETTE = [
...
class HomePage(tk.Tk):
    def __init__(self, user):
        super().__init__()
        self.user, self.modulecs, self.widgets = user, [], []
    ...
    def load_data(self):
        if os.path.exists("modulecs.json"):
            try:
                with open("modulecs.json", "r", encoding='utf-8') as f:
                    for m in json.load(f).get("modulecs", []): self.modulecs.append(Module(m))
            except: pass
        if not self.modulecs:
            for n in ["Dynamique des Fluides & Turbomachines",
            " Systèmes d’Information & Bases de Données Rela",
            "Dessin Industriel & CAO",
            "POO Python",
            " CONCEPTION ELECTRONIQUE INDUSTRIELLE",
            " Optimisation & Analyse des Données ",
            "TEC"]:
                self.modulecs.append(Module(n))

    def save_data(self):
        with open("modulecs.json", "w", encoding='utf-8') as f:
            json.dump({"modulecs": [m.name for m in self.modulecs]}, f, indent=2)
    ...
    def refresh(self):
        ...
        for i, m in enumerate(self.modulecs):
            # On utilise le modulo (%) pour boucler sur la palette si on a plus de modulecs que de couleurs
    ...
    def add_mod(self):
        if (n := simpledialog.askstring("Add", "Name:")):
            self.modulecs.append(Module(n.strip())); self.save_data(); self.refresh()
